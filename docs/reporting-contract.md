...
這三件不補，契約收得到資料但**沒有診斷價值**——欄位是滿的，內容是空的。

### 1. 環境檢查失敗時保留 stderr（最重要）

**現況**：`src/env-check.js` 的 `runProbe` 有抓 stderr（`child.stderr.on("data", …)`），
但呼叫端全部丟掉，換成一個形容詞：
...
同樣的寫法在 `env-check.js` 出現六次（`檢查失敗` / `無法判讀登入狀態` /
`無法確認`）。**錯誤被換成形容詞，原因當場消失。**

而「檢查一直不過」正是學生最常卡住的地方——最需要資訊的情境，剛好是資訊最少的。

**要做的**：失敗路徑多留一個欄位（例如 `debugDetail`），放遮罩後的 stderr 前 500 字。
**畫面上顯示的文案不變**（學生看到的還是「檢查失敗」），那個欄位只進回報。

**不補的後果**：指紋庫會塞滿一堆指紋相同、內容都是「檢查失敗」的紀錄——等於沒有
知識庫。這是三件裡唯一會讓整個系統失去意義的。

**影響契約的**：`submit-log` 的 `env.checks[]`（現在只有狀態沒有原因）。

### 2. raw output 保留最近三次執行

**現況**：`public/app.js` 在每次執行開始時呼叫 `view.clearRawOutput()`，所以只留
**最後一次**。

學生的實際行為是「試了三次都不行才求救」——你收到的是第三次，而前兩次可能才是有
線索的那次（例如第一次的錯誤訊息比較完整，後面兩次只剩「已存在」）。

**要做的**：`rawOutputs` 改成每張卡留最近三次，`submit-log` 一起送。

### 3. 附上這張卡的操作時間軸

**現況**：完全沒有。收到的是一個時間點的快照。

有些問題是**順序**造成的——最典型的是沒先放行 PowerShell 執行原則就去按後面的卡，
每一張都失敗，而每一張的錯誤訊息看起來都跟執行原則無關。只看最後一次診斷不出來。

**要做的**：記下這張卡上按過哪些動作、各自的時間與結果（不含輸出內容，只有
`action` / `at` / `exit_code`），`submit-log` 一起送。

### 順序

1 先做，它是唯一會讓知識庫失效的。2 和 3 是「讓難查的案例查得動」，可以等第一批
真實資料進來之後再決定要不要做。
