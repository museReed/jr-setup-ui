...
- **類型**：continuation
- **分支**：`rework/returning-students`（644 項測試綠）
...
| 項目 | 為什麼 | 什麼時候能驗 |
|---|---|---|
| C2 的「`merge-backups` 不被誤傷」 | 這台從沒跑過合併，那個目錄根本不存在 | 走到規則段、按過「用 AI 合併」之後 |
| A7a 規則段那一半 | 要走完整個規則段才到得了最後一張 | 同上 |
...
⚠️ 另外順手修掉一個同族的：**還沒裝的卡會提前顯示自救步驟**。`guidanceModel`
原本 `missing` 也給 guidance，而 GUIDANCE 每一段的文案都假設「已經裝了、但不生效」
——分頁標題那張 0/3 就在講「名字已經寫進同步檔，但標題沒有動」，講的是還沒發生的
事。改成只有 `warn`（或動作真的失敗）才給。
