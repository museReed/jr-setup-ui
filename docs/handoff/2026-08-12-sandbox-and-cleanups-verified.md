...
- **類型**：continuation
- **分支**：`rework/returning-students`＝`main`（兩條指向同一個 commit，647 項測試綠）
...
6. **`main` 已經被 reset 成這條分支**（見最後一節）。兩條 branch 現在指向同一個
   commit，學生走的 `irm … | iex`（不設 `$JrBranch`）拿到的就是這份程式碼。
7. **合併的完成判準換掉了**——不再靠 agent 自己寫標記檔（見最後一節）。
8. ⚠️ **收工前發現一個還沒修的**：一張卡裝兩份時，自動接的驗證只驗最後那一份。
   完整診斷在最後一節的「下一輪第一件事」。
...
- **UAC 的預設按鈕永遠是「否」**。今天在三個地方撞到（codex 沙箱設定、msiexec、
  換 pwsh）。學生順手按 Enter 就等於取消，而畫面上不會有任何說明。
- 前一份的「已知問題」全部仍然成立。
...
`main` 原本有 56 個 commit 是這條分支沒有的（分支從 `51c3e96` 另開，刻意不從 main
複製 code）。Reed 決定放棄 main 那條線，`main` 現在直接指向這條分支。

- **舊 main 的 tip 是 `646d6e1`**，本機物件庫裡還撈得到：`git show 646d6e1:<路徑>`
- reset 之前撿了三樣（見上一節「從 main 搬過來的」）
- ⚠️ **`docs/setup.ps1` 永遠從 main 抓**，所以 push 的那一刻起學生拿到的就是這份。
  分支的版本比舊 main 多兩段修正（先關掉還開著的舊嚮導、下載時關進度列），是進步
  不是回退
- 之後兩條 branch 一起推：`git push origin main` 再 `git push origin main:rework/returning-students`

### 合併的完成判準換掉了（`a78b0c9`）

早上補的退路（「檔案變了 + 安靜五秒」）在 VM 上兩次都正確接手，但**舊 main 的做法
更好**，整段移植過來：

| | 早上做的 | 現在 |
|---|---|---|
| 判準 | 檔案動了沒 | **`missingSourceLines`**——工作坊那段每一行是不是真的進去了 |
| 用誰的判準 | 自己一套 | **跟畫面上那一列同一支函式** |
| 逾時 | 只說「等太久了」 | 把缺的那幾行**整段印出來**，分檔、不截斷 |

差別在「檔案動了」與「內容對不對」：AI 只併了一半的話，前者判成完成，後者會繼續等
並告訴你差哪幾行。

⚠️ **提示詞裡「工作坊那段請一字不差整段貼進去」那一句跟判準是綁死的**，不要為了
讀起來順而拿掉：判準逐行比對，而 AI 合併本來就會潤飾——把兩行併成一行就對不上，
於是它說做完了、嚮導永遠等不到。

⚠️ **這個改動還沒在 VM 上驗過。** 新判準比舊的嚴格，AI 潤飾了工作坊那段就會等到
逾時——**那是預期行為**。要驗的是「缺行清單貼回終端，agent 補完之後嚮導會自己往下走」。

### 其餘四個（都是畫面上看得出來的）

| commit | 內容 |
|---|---|
| `f1926ac` | 「Windows 先準備好」不再寫死「四件事」——選了 Codex 是五列 |
| `6d33b59` | **規則卡的按鈕掛回它負責的那一列**，不再全部落在卡片底部（Reed 指定） |

⚠️ `6d33b59` 有個陷阱值得知道：`view.js` 的 `inlineActions` 是一張 Map，`rowId` 算錯
的話按鈕**不是跑錯位置，是安靜地消失**。拆不拆成「安裝／驗證」兩格的規則抽成
`checklistRowIds` 跟 `checklistGroups` 共用，並補了守門測試走訪四種代表性的列。

## 下一輪第一件事：自動驗證只驗最後裝完的那一份

**症狀**（VM 實測，「它什麼時候該停下來問你」那張卡）：按安裝之後，畫面上
「驗證：一次只跑一個指令」自動打勾了，而「驗證：常用指令不用每次問你」還空著——
清單看起來順序不對。

**根因**：那張卡按安裝會**依序裝兩份**（`nextInstallStep`），而 `app.js:1562` 自動接
的驗證用的是 `installedCheck`——**最後一個裝完的那一份**。前面那一份的驗證從來沒被
觸發過。

⚠️ **這個 bug 在 8/12 早上換順序之前就存在**，只是方向相反：先前 `allowlist` 排最後，
所以自動驗白名單、擋串接那格沒驗。換順序（`cb8b49e`）只是把「沒被自動驗到的那一格」
換了個人。**不要以為是換順序造成的而把順序改回去，那不會解決問題。**

**兩種修法：**

| | 做法 | 代價 |
|---|---|---|
| A | 自動驗**第一格**沒驗過的 | 小改動，但另一格還是要手動按——只是把問題換一格 |
| B | 兩格**依序都自動驗** | 正確，但兩次驗證各跑一分多鐘，串起來要等兩分鐘 |

建議 **B**，但要加「現在在驗第幾格」的提示，不然畫面看起來像當掉。

⚠️ 這一段（安裝 → 驗證的串接）是這條分支最容易出事的地方，8/12 一天就在同一段踩了
三次（`ef6ed37`、`9884ca2`、`83d9de1`）。改完**必須在 VM 上重跑整張卡**。

## VM 驗收走到哪

8/12 傍晚從乾淨的 dirty 快照重跑了一次，**跑的是學生真正的路徑**（不設 `$JrBranch`、
從 main 抓），確認 `.jr-source` 是 `main` 且拿到最新程式碼。

已驗到「Windows 先準備好」那張全綠（含 `pwsh-store` 一鍵換成一般安裝版，**按完直接
轉綠、不用重開嚮導**）。之後的步驟清單在對話裡，重點是：

1. 環境段其餘六顆按鈕（清舊捷徑、清 npm、搬 skill、接沙箱、gh、claude）
2. **「清掉搬走的備份」那張新卡**——按完要**留著打勾**，不是消失
3. 環境段最後一張「這一段已完成」時**不該**再自動重查
4. 規則段：還沒裝的卡**不該**顯示自救說明；等合併的列**不該**被打勾
5. **合併走新判準**（見上面，這是最重要的一項）
6. 筆記那兩張卡只有兩格，沒有假的「驗證」格
7. 回報鈴鐺（8/11 就欠著，一直沒驗到）

## 還沒處理的缺口

- **Claude 的 skill 覆蓋不是百分之百乾淨**：安裝只寫我們現在發的那幾個檔案，舊版
  留下的其他附屬檔會留在同一個資料夾裡，而 Claude 載入 skill 看的是整個資料夾。
  舊 main 有處理（「先搬到隔離區，再裝乾淨的」），**這條分支沒有**。實務上影響小
  （目前只有 `vault-sync` 多一個 references），但那是個真的缺口。
- **`.jr-source` 只記分支名不記 commit**——「我跑的是不是最新的」問不出來，今天為此
  繞了三四輪，最後靠 grep 一個 ASCII 識別字才確定。加一行 sha 就解決。
- **`reset-codex-install.ps1` 沒有任何人找得到**。救火工具只寫在交接裡。
