...
//
// 只講一次、只回報、不自動刪：學生可能改過那支 skill，或還想留著。每次重查都印一次
// 的話，那句話會在終端裡洗版，反而沒人看。
...
function straySkillNotice() {
  ...
  return {
    text: `這台機器上還留著 ${state.straySkills.length} 個工作坊已經停發的 skill：${names}`,
    detail:
      "它們是上一輪工作坊裝的，這一輪已經不發了，但 Claude 仍然會載入它們。按下面那顆會把它搬到隔離區——不是刪除，搬錯了可以搬回來。你自己裝的 skill 不會出現在這個清單裡。完整路徑在「看原始輸出」裡。",
    // 一個一個來，沒有「全部搬走」。
    //
    // 清單本身現在已經只認我們自己發過的名字（見 legacy.js 的 RETIRED_SKILLS），
    // 誤傷的風險小了很多——但仍然一個一個：搬走是不可逆感很強的動作，而學生可能
    // 對其中某一支還有感情（例如他改過它）。
