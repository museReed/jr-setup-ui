...
const CASES = {
  ...
  "skill-questions": {
    ...
    //
    // ⚠️ 題目刻意跟電腦無關（Reed 指定改成日本旅遊）。原本問的是「幫這台電腦選一個
    // 終端機工具」，兩個問題：那是學生此刻正在做的事，他會分不清這是驗證還是嚮導
    // 真的要他選；而且答案聽起來像設定，答錯了會怕。旅遊規劃沒有正確答案，怎麼選
    // 都不會弄壞什麼——他才有餘裕去看「這個提問介面長什麼樣」，那正是這一格要他學的。
    prompt: ({ agent }) =>
      (agent === "codex" ? "$structured-questions " : "") +
      "我想規劃一趟日本旅行，但還沒想好要怎麼安排。" +
