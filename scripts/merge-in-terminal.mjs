...
const prompt = [
  ...
  // ⚠️ 「一字不差」跟 TOML 的重複鍵會打架，而那是必然會發生的：學生上次上課裝的
  // 就是同一份設定，兩邊一模一樣。照字面貼進去 config.toml 會出現重複鍵、Codex
  // 直接載不進去；不貼又違反上面那條。VM 實測 Codex 停在那裡列了三個選項問人，
  // 而那三個選項其實都是對的——它只是不知道自己可以決定。
  //
  // 放行的根據：判準是 missingSourceLines，逐行問「這一行在不在」（Set 比對），
  // **不看有幾份**。所以留一份就算完成，刪掉重複的那份不影響。
  "⚠️ 兩邊完全相同的行，保留一份就好，不要貼成兩份——TOML 有重複鍵會讓 Codex " +
    "載不進設定。系統只看那一行在不在，不看有幾份，所以留一份就算完成。",
